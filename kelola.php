    ...
    if ($_SESSION['admin'] != 1 and $_SESSION['admin'] != 2) {
        ...
        if (isset($_REQUEST['sub'])) {
            ...
            if (isset($_REQUEST['submit'])) {
                ...
                if (
                    $_REQUEST['cabang'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['divisi'] == "" || $_REQUEST['alamat'] == "" || $_REQUEST['kepcab'] == "" || $_REQUEST['nip'] == ""
                    || $_REQUEST['website'] == "" || $_REQUEST['email'] == ""
                ) {
                    $_SESSION['errEmpty'] = 'ERROR! Semua form wajib diisi';
                    header("Location: ././admin.php?page=kel");
                    die();
                } else {
                    ...
                    if (!preg_match("/^[a-zA-Z0-9. -]*$/", $nama)) {
                        ...
                        if (!preg_match("/^[a-zA-Z0-9.,&() \/ -]*$/", $cabang)) {
                            $_SESSION['cabang'] = 'Form Nama Yayasan hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan minus(-)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {

                            if (!preg_match("/^[a-zA-Z0-9.,:\/<> -\"]*$/", $divisi)) {
                                $_SESSION['divisi'] = 'Form divisi hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), titik dua(:), petik dua(""), garis miring(/) dan minus(-)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {
                                ...
                                if (!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $alamat)) {
                                    ...
                                    if (!preg_match("/^[a-zA-Z., ]*$/", $kepcab)) {
                                        $_SESSION['kepcab'] = 'Form Nama Kepala Sekolah hanya boleh mengandung karakter huruf, spasi, titik(.) dan koma(,)<br/><br/>';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if (!preg_match("/^[0-9 -]*$/", $nip)) {
                                            $_SESSION['nipkepcab'] = 'Form NIP Kepala Sekolah hanya boleh mengandung karakter angka, spasi, dan minus(-)<br/><br/>';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {
                                                    ...
                                                    if (!empty($logo)) {
                                                        ...
                                                        if (in_array($eks, $ekstensi) == true) {
                                                            if ($ukuran < 2000000) {
                                                                ...
                                                                $query = mysqli_query($config, "UPDATE perusahaan SET cabang='$cabang',nama='$nama',divisi='$divisi',alamat='$alamat',kepcab='$kepcab',nip='$nip',website='$website',email='$email',logo='$nlogo',id_user='$id_user' WHERE id_cabang='$id_cabang'");
                                                        ...
                                                        $query = mysqli_query($config, "UPDATE perusahaan SET cabang='$cabang',nama='$nama',divisi='$divisi',alamat='$alamat',kepcab='$kepcab',nip='$nip',website='$website',email='$email',id_user='$id_user' WHERE id_cabang='$id_cabang'");
                ...
                if (mysqli_num_rows($query) > 0) {
                    ...
                    while ($row = mysqli_fetch_array($query)) { ?>
                            ...
                            <div class="col s12">
                                <nav class="secondary-nav">
                                    <div class="nav-wrapper blue-grey darken-1">
                                        <ul class="left">
                                            <li class="waves-effect waves-light"><a href="?page=kel" class="judul"><i class="material-icons">work</i>Edit Profil Perusahaan</a></li>
                                        </ul>
                                    </div>
                                </nav>
                            </div>
                        ...
                        <div class="row jarak-form">
                            ...
                            <!-- Form START -->
                            <form class="col s12" method="post" action="?page=kel" enctype="multipart/form-data">

                                <!-- Row in form START -->
                                <div class="row">
                                    <div class="input-field col s6">
                                        <input type="hidden" value="<?php echo $id_cabang; ?>" name="id_cabang">
                                    ...
                                    <div class="input-field col s6">
                                        <input id="cabang" type="text" class="validate" name="cabang" value="<?php echo $row['cabang']; ?>" required>
                                        <?php
                                        if (isset($_SESSION['cabang'])) {
                                            $cabang = $_SESSION['cabang'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $cabang . '</div>';
                                    ...
                                    <div class="input-field col s6">
                                        <input id="kepcab" type="text" class="validate" name="kepcab" value="<?php echo $row['kepcab']; ?>" required>
                                        <?php
                                        if (isset($_SESSION['kepcab'])) {
                                            $kepcab = $_SESSION['kepcab'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $kepcab . '</div>';
                                            unset($_SESSION['kepcab']);
                                        }
                                        ?>
                                        <label for="kepcab">Nama Kepala Cabang</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <?php
                                        if (isset($_SESSION['nipkepcab'])) {
                                            $nipkepcab = $_SESSION['nipkepcab'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $nipkepcab . '</div>';
                                            unset($_SESSION['nipkepcab']);
                                        }
                                        ?>
